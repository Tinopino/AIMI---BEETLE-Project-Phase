...
class HardMiningBatchReferenceSampler(BatchReferenceSampler):
    ...
    def _build_reference_alias_map(self) -> Dict[str, object]:
        """
        Map robust slide aliases to a WholeSlideData sample reference.

        WholeSlideData file_key values are internal association keys and do not
        necessarily match the human-readable slide stems stored in the mined
        manifest. Therefore we also index the real WSI path and annotation path.
        """
        ...
        for sample_references in self._dataset.sample_references.values():
            for reference in sample_references:
                values = [reference.file_key]

                try:
                    wsi = self._dataset.get_wsi_from_reference(reference)
                    values.extend([
                        getattr(wsi, "path", None),
                        str(wsi),
                    ])
                except Exception:
                    pass
                ...
                try:
                    wsa = self._dataset.get_wsa_from_reference(reference)
                    values.extend([
                        getattr(wsa, "path", None),
                        str(wsa),
                    ])
                except Exception:
                    pass
        ...
        print(
            "[HARD MINING] Built reference alias map with "
